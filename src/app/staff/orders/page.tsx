...
export default function StaffOrdersPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'on_hold': return <Badge className="bg-amber-100 text-amber-700 hover:bg-amber-100 border-none">On Hold</Badge>;
      default: return <Badge className="bg-slate-100 text-slate-700 border-none capitalize">{status}</Badge>;
    }
  };
      ...
      <div className="flex flex-col gap-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-500">View and print all orders.</p>
          </div>
        ...
        {filteredOrders.length > 0 && (
          <div className="p-6 border-t bg-slate-50 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 order-1 sm:order-2">
              <Button variant="outline" size="icon" onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="rounded-lg h-9 w-9 border-slate-200 bg-white">
                  ...
                  .map((p, i, arr) => (
                    <div key={p} className="flex items-center gap-1">
                      ...
                      <Button
                        ...
                        className={cn("h-9 w-9 p-0 rounded-lg font-bold transition-all", currentPage === p ? "bg-primary hover:bg-primary/90 text-primary-foreground shadow-md" : "text-slate-600 hover:bg-slate-200")}
                      >
                        {p}
                      </Button>
                    </div>
              ...
              <Button variant="outline" size="icon" onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages || totalPages === 0} className="rounded-lg h-9 w-9 border-slate-200 bg-white">
